...
#G is the embedded NetworkX DiGraph
...
#Coord_source is a dictionary assigning NumPy arrays of d elements containing the Cartesian coordinates of the network nodes as sources in a d-dimensional space to the node names.
#Coord_target is a dictionary assigning NumPy arrays of d elements containing the Cartesian coordinates of the network nodes as targets in a d-dimensional space to the node names.
...
#   [GRscore_hyp,percentageOfSuccPaths_hyp,avgGRlength_hyp,avgSHlength_hyp]=GRcalculation.greedy_routing(G,Coord_source_hyp,Coord_target_hyp,'hypDist')
def greedy_routing(G,examinedNodePairsDict,Coord_source,Coord_target,usedMeasure,K=-1):
    ...
    for startNode in Coord_source.keys(): #iterate over those nodes that have non-zero out-degree (i.e., that serve as source of links)
        for endNode in examinedEndPoints: #iterate over those nodes that are present in the examined paths as endpoints (these always have a position as a target, since all the examined paths are finite)
    ...
    if usedMeasure=='innerProd':
        for (startNode,endNode) in examinedNodePairsDict.keys():
            ...
            if math.isnan(GRpathsDict[(startNode,endNode)]): #we do not know yet
                greedy_routing_rec_EucInner(startNode, endNode, G, Coord_target, measureDict, closestDict, GRpathsDict)
    elif usedMeasure=='EucDist':
        for (startNode,endNode) in examinedNodePairsDict.keys():
            #check whether we already know the greedy path length from node startNode to node endNode
            if math.isnan(GRpathsDict[(startNode,endNode)]): #we do not know yet
                greedy_routing_rec_EucDist(startNode, endNode, G, Coord_target, measureDict, closestDict, GRpathsDict)
    elif usedMeasure=='hypDist':
        for (startNode,endNode) in examinedNodePairsDict.keys():
            #check whether we already know the greedy path length from node startNode to node endNode
            if math.isnan(GRpathsDict[(startNode,endNode)]): #we do not know yet
                greedy_routing_rec_hypDist(startNode, endNode, G, Coord_target, measureDict, closestDict, GRpathsDict, K)
    else:
...
def greedy_routing_rec_EucInner(i, j, G, targetCoords, measures, closestNeighbors, GRpaths): #determine the length of the GR path from node i to node j
        ...
        for neighborNode in G.successors(i):
            ...
            if measures[(neighborNode,j)]<bestMeasure:
                ...
                if rand.random()<0.5: #sample a floating point number in the range [0.0, 1.0)
                    closestNeighbors[(i,j)] = neighborNode
        if math.isinf(bestMeasure): #node i does not have any successors
    ...
    if closestNeighbors[(i,j)]==j: #the node following node i is the endpoint
        GRpaths[(i,j)] = 1
    elif G.out_degree(closestNeighbors[(i,j)])==0: #the node following the ith node has no outgoing link (it is a dead end)
        ...
        if math.isnan(GRpaths[(closestNeighbors[(i,j)],j)]): #the node following node i has not been visited yet and we do not know the length of the path from it to the endpoint
            greedy_routing_rec_EucInner(closestNeighbors[(i,j)], j, G, targetCoords, measures, closestNeighbors, GRpaths)
...
def greedy_routing_rec_EucDist(i, j, G, targetCoords, measures, closestNeighbors, GRpaths): #determine the length of the GR path from node i to node j
    GRpaths[(i,j)] = -1 #mark that the ith node (the starting node) has already been visited in the current path
    if math.isnan(closestNeighbors[(i,j)]): #we have not determined which neighbor of node i is the closest on to node j
        bestMeasure = math.inf
        for neighborNode in G.successors(i):
            measures[(neighborNode,j)] = np.linalg.norm(targetCoords[neighborNode]-targetCoords[j]) #greedy routing: always step to the node neighbour minimising this measure, i.e. to the one that has the smallest Euclidean distance from the destination
            if measures[(neighborNode,j)]<bestMeasure:
                closestNeighbors[(i,j)] = neighborNode
                bestMeasure = measures[(neighborNode,j)]
            elif measures[(neighborNode,j)]==bestMeasure:
    ...
    if closestNeighbors[(i,j)]==j: #the node following node i is the endpoint
        ...
        if math.isnan(GRpaths[(closestNeighbors[(i,j)],j)]): #the node following node i has not been visited yet and we do not know the length of the path from it to the endpoint
            greedy_routing_rec_EucDist(closestNeighbors[(i,j)], j, G, targetCoords, measures, closestNeighbors, GRpaths)
...
def greedy_routing_rec_hypDist(i, j, G, targetCoords, measures, closestNeighbors, GRpaths, K=-1): #determine the length of the GR path from node i to node j
    GRpaths[(i,j)] = -1 #mark that the ith node (the starting node) has already been visited in the current path
    if math.isnan(closestNeighbors[(i,j)]): #we have not determined which neighbor of node i is the closest on to node j
        bestMeasure = math.inf
        for neighborNode in G.successors(i):
            measures[(neighborNode,j)] = hypDist(targetCoords[neighborNode],targetCoords[j],K) #greedy routing: always step to the node neighbour minimising this measure, i.e. to the one that is the hyperbolically closest to the destination
            if measures[(neighborNode,j)]<bestMeasure:
                closestNeighbors[(i,j)] = neighborNode
                bestMeasure = measures[(neighborNode,j)]
            elif measures[(neighborNode,j)]==bestMeasure:
    ...
    if closestNeighbors[(i,j)]==j: #the node following node i is the endpoint
        ...
        if math.isnan(GRpaths[(closestNeighbors[(i,j)],j)]): #the node following node i has not been visited yet and we do not know the length of the path from it to the endpoint
            greedy_routing_rec_hypDist(closestNeighbors[(i,j)], j, G, targetCoords, measures, closestNeighbors, GRpaths, K)
